...
class Job(object):
    ...
    def run_excel(self,*args,**kwargs):

        df_res=kwargs.get('df_res')
        out_path = kwargs.get('out_path','./output.xlsx')
        head = df_res.columns.values.tolist()
        IoJob().df_write_to_excel(path=out_path,head=head,df=df_res)




    def run__email(self,*args,**kwargs):
        in_path = kwargs.get('in_path', '../files/input.xlsx')
        out_file_name=kwargs.get('file_name','output.xlsx')
        df_res = kwargs.get('df_res')
